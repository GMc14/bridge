...
function gamesaddPlayer(gameCode, player) {
  check(player, String);
  const result = (Games.findOne({
    gameCode: gameCode
  }));
  Games.update(result._id, {
    $push: {
      player: {
        name: player
      }
...
function gamesShuffle(gameCode) {
...
function gamesStartGame(gameCode) {
  check(gameCode, String);
  const game = (Games.findOne({
    gameCode: gameCode
  }));
...
function gamesToggleTimer(gameCode) {
  check(gameCode, String);
  const game = (Games.findOne({
    gameCode: gameCode
  }));
...
function gamesNextRound(gameCode) {
  check(gameCode, String);
  const game = (Games.findOne({
    gameCode: gameCode
  }));
  ...
  Games.update(game._id, {
    $set: {
      timeLeft: 180000 - game.round * 60000,
      round: game.round + 1
    }
  });
