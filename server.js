...
io.sockets.on('connection', function(socket) {
  ...
  socket.on('selPlayer', function(data) {
    ...
    console.log(pLeft);
    io.sockets.to(data.roomID).emit('playerDataToClient', {nickname:socket.nickname, player:playerNum, pLeft:pLeft});
  ...
  socket.on('dealCards', function(data) {
      io.sockets.to(data.roomID).emit('deal', {Hand1:data.Hand1, Hand2:data.Hand2, Hand3:data.Hand3, Hand4:data.Hand4});
  });
  socket.on('playCard', function(data) {
    io.sockets.to(data.roomID).emit('cardPlayed',{player:socket.player, card:data.card});
  });
